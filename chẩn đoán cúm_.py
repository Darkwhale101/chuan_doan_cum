...
class FluDiagnosisApp:
    ...
    def create_widgets(self):
        ...
        form_title = Label(left_frame, text="📋 THÔNG TIN TRIỆU CHỨNG", font=("Arial", 16, "bold"), bg='white', fg='#34495e')
        ...
        Label(symptoms_frame, text="Các triệu chứng khác:", font=("Arial", 12, "bold"), bg='white', fg='#2c3e50').pack(anchor=W, pady=(0, 10))
        ...
        self.diagnose_btn = Button(btn_frame, text="🔍 CHẨN ĐOÁN", command=self.diagnose, font=("Arial", 14, "bold"), bg='#3498db', fg='white', padx=30, pady=12, cursor='hand2', relief=RAISED, borderwidth=2)
        ...
        self.reset_btn = Button(btn_frame, text="🔄 LÀM MỚI", command=self.reset_form, font=("Arial", 12), bg='#95a5a6', fg='white', padx=20, pady=8, cursor='hand2')
...
• Nếu có triệu chứng nghiêm trọng, hãy đến gặp bác sĩ"""
        Label(info_frame, text=info_text, font=("Arial", 10), bg='#ecf0f1', fg='#34495e', justify=LEFT).pack(padx=10,pady=10)
    def create_temperature_input(self, parent):
        temp_frame = Frame(parent, bg='white')
        temp_frame.pack(fill=X, padx=20, pady=10)
        Label(temp_frame, text="🌡️ Nhiệt độ cơ thể (°C):", font=("Arial", 12, "bold"), bg='white', fg='#2c3e50').pack(anchor=W)
        ...
        self.temp_display = Label(temp_frame, text="37.0°C", font=("Arial", 24, "bold"), bg='white', fg=self.get_temp_color(37.0))
    ...
    def create_symptom_checkbox(self, parent, text, variable):
        ...
        cb = Checkbutton(cb_frame, text=text, variable=variable, font=("Arial", 11), bg='white', activebackground='#ecf0f1', selectcolor='white', cursor='hand2')
        cb.pack(anchor=W)
    ...
    def show_positive_result(self):
        result_icon = Label(self.result_frame, text="⚠️", font=("Arial", 48), bg='white', fg='#e74c3c')
        result_icon.pack(pady=10)
        result_text = Label(self.result_frame, text="KẾT QUẢ: CÓ KHẢ NĂNG BỊ CÚM", font=("Arial", 18, "bold"), bg='white', fg='#e74c3c')
...
• Tránh đến nơi đông người"""
        Label(advice_frame, text=advice, font=("Arial", 11), bg='#ffe5e5', fg='#c0392b', justify=LEFT).pack(padx=15, pady=15)
    def show_negative_result(self):
        result_icon = Label(self.result_frame, text="✅", font=("Arial", 48), bg='white', fg='#27ae60')
        result_icon.pack(pady=10)
        result_text = Label(self.result_frame, text="KẾT QUẢ: KHÔNG CÓ DẤU HIỆU CÚM", font=("Arial", 18, "bold"), bg='white', fg='#27ae60')
        result_text.pack(pady=5)
...
• Theo dõi sức khỏe định kỳ"""
    ...
    def show_symptom_summary(self, features, actual_temp):
        ...
        Label(summary_frame, text="📊 Tóm tắt triệu chứng:", font=("Arial", 12, "bold"), bg='#f8f9fa', fg='#2c3e50').pack(anchor=W, padx=10, pady=5)
        symptoms = [
        ...
        for symptom, value in symptoms:
            ...
            Label(summary_frame, text=f"  • {symptom}: {status}", font=("Arial", 10), bg='#f8f9fa', fg=color).pack(anchor=W, padx=20)
        total_symptoms = sum(features)
        Label(summary_frame, text=f"\nTổng số triệu chứng: {total_symptoms}/6", font=("Arial", 11, "bold"), bg='#f8f9fa', fg='#34495e').pack(pady=5)
    def reset_form(self):
